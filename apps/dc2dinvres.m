...
% end
% set(gcf,'Pointer','arrow');
% RMS=[RMS rrms(N.r,Mod.R,INV.lolo)];
...
if ~isempty(nr),
    ...
    if strcmp(questdlg(mess,'Delete Datum?','Yes','No','No'),'Yes'),
        ...
        RMS(end)=rrms(N.r,Mod.R,INV.lolo);
...
hold off
if INV.lolo,
    ss=sprintf('Model Difference RMS = %.2f%%',rrms(log(Mod.M(:)),log(M1(:))));
else
    ss=sprintf('Model Difference RMS = %.2f%%',rrms(Mod.M(:),M1(:)));
end
...
%     iprms=rrms(N.ip+eps,Mod.ipfor,0);
...
if fname~=0,
    ...
    if isfield(N,'topo'),
        ...
        T.a=jj(N.a);T.b=jj(N.b);T.m=jj(N.m);T.n=jj(N.n);
...
%         Mod.Cov=zeros(size(Mod.M));Mod.Cov(:)=sum(abs(S));
        ...
        appendmessg(sprintf('(%.1f%%)',rrms(NN.r,RR,1)));
