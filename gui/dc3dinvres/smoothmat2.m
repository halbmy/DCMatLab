function C=smoothmat2(Model,rb)

% SMOOTHMAT - matrix for (horizontal) smoothness constraints of 2nd order
% C = smoothmat2(Model)

if nargin<2, rb=0; end
nnz=0;
...
for k=1:length(Model.M),
    ...
    if nx*ny>1,
        C2=smooth2d2nd(0:nx,0:ny,rb);
        new=old+nx*ny;
        C(old+1:new,old+1:new)=C2;
        old=new;
    end
end
