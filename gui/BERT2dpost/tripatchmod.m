...
if isfield(MAL,'clust')&&(MAL.clust>0),
    ...
    if (MAL.clust>1)&&(MAL.clust<4), MAL.cmax=MAL.clust*1.5-0.5;MAL.cmin=1; end
end
if ~isfield(MAL,'oldstyle'), MAL.oldstyle=0; end
...
if isfield(MAL,'oldstyle')&&(MAL.oldstyle>0),
    for i=1:size(cols,1), 
        ...
        patch(Mesh.node(cnodes,1),Mesh.node(cnodes,2),cols(i,:),'LineStyle','none'); 
