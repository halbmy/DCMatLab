...
set(handles.text13,'String','CylBERT 0.5');
...
%     N.r=N.rho.*N.konf; % alt
    %% auswählen welche (wie timelapse)!!!
    [tf,loc]=ismember([N.a(:) N.b(:) N.m(:) N.n(:)],abmn,'rows');
    fo=find(loc);fo0=find(tf==0);
    N.r=zeros(size(N.a));
    N.r(fo)=N.rho(fo).*DD.k(loc(fo));
    if i==1, 
        N.k=ones(size(N.a))*(-1e8);
        N.k(fo)=DD.k(loc(fo));
    end
%     minmax(N.r)
    fi=find((N.r<minrhoa)|(N.err>maxerr/100)|(abs(N.ip)>maxip));
...
%     N.rho(fi)=N.r(fi)./N.konf(fi);
    ...
    saveinv3dfile(fullfile(dirname,fnames{i}),N);%rmfield(N,{'r','k'}));
...
%         cmdline=['dcinv -vvL -p meshPara.bms ' opts '0.dat']
    end
    dos(cmdline);
...
%     dos(['copy /y ' fullfile(dirname,'dcinv.result.vtk') ' "' fullfile(pp,'0.vtk') '"']);
    ...
    out=sprintf('Model 0: min/max=%.1f/%.1f',min(Mesh.cellattr),max(Mesh.cellattr));    
    ...
    for i=1:size(ABS,1),
        Mesh.cellattr=ABS(i,:)';
        savevtkmesh(Mesh,fullfile(pp,[num2str(i-1) '.vtk']));
    end
catch,
    cd(oldpwd);
end
