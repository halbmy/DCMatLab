...
if islogical(feld), feld=double(feld)+1; end
if nargin<3,
...
%     if (floor(dd*20)~=dd*20)|(length(unique(di))>size(N.elec,1)/5), del=1; end
    ...
    caxis([cmin cmax]);
    switch cmap
        case 1, colormap(flipud(parula));
        case 2, colormap(b2r);
        case 3, colormap(hot);
        case 4, colormap(gray);
        case 5, colormap(jet);
        case 6, colormap(cool);
        otherwise, colormap(parula); % keep parula
    end
...
%     yl=get(gca,'YLim');set(gca,'YTick',yl(1):yl(end)); % Test to see all
    ...
    if cbar,
        hc=colorbar('horiz');
%         mver = ver('MATLAB');
        try % new matlab engine (>=R2014b)
            cpos=hc.Position;
%             cpos(4)=cpos(4)*0.5;
%             cpos(2)=(cpos(2)-cpos(4))/2+cpos(4);
%             cpos(4)=cpos(4)-0.02;
%             hc.Position=cpos;
        catch  % old matlab engine
