function [N,XX,EL,SP] = readres2dinvfile(datafile)
...
if (typ==11)||(typ==12)||(typ==13), % mixed array
    ...
    nntyp=[3 3 4 3 3 4 4 0 0 0 8 10]; %No. of columns
    nn=nntyp(typ);
    ss='%f';
    ...
    for l=1:data,
        ...
        zeile(zeile==',')=' ';
%         DATA(:,l)=sscanf(zeile,ss,[nn+ippresent,data]);
        DATA(:,l)=sscanf(zeile,ss,[nn+ippresent+iserr,data]);
    end
    istopo=fscanf(input,'%d',1);
    if istopo,
       nrtopo=fscanf(input,'%d\n',1);
...
%        N.topo=fscanf(input,'%f',[2 nrtopo])';
    ...
    if nn==4, % 4-column-data (DD,PD,SL)
...
%         AA=XX-location*SP.*EL-(SP<0).*(SP-1).*EL;
        AA=XX-location*(SP-1).*EL/2;%-(SP<0).*(SP-1).*EL;
        MM=AA+SP.*EL;
        NN=MM+sign(SP).*EL;
