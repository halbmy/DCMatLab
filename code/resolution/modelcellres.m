...
scal=1;
if size(S,2)==size(L,2), % transposed jacobian matrix
    b=S(:,n);
...
elseif size(S,1)==size(L,2), % normal jacobian matrix
    b=S(n,:);
    x=zeros(size(S,1),1);x(n)=1;
    PI = P\speye(size(P,1));
    x=PI*x;
    z = D*(b - ((P*x)'*S)'); % residuum of unregularized equation
    p = PI*(S*(D*z))-PI*(L*(P*x))*lam; % residuum of normal equation
    r = p;normr2 = r'*r;
    acc=1e-4;
    abbr = normr2*acc; % goal for norm(r)^2
    j=0;
