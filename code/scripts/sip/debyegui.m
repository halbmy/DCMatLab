...
kfak=0.04^2*pi/4/0.07;
% kfak=str2num('1/39.8'); %alte Zelle ( 1/(0.04^2*pi/4/0.05)
...
yl=ylim;
% tmed=tau(round(median(m)))
summ=cumsum(m)/sum(m);
t50=exp(interp1(summ,log(tau),0.5));
line(t50*[1 1],yl,'Color','g');
t=text(t50,yl(1),num2str(rndig(t50*1000),'%g ms'),'Color','green');
set(t,'HorizontalAlignment','center','VerticalAlignment','top');
tlm=exp(sum(log(tau).*m)/sum(m));
line(tlm*[1 1],yl,'Color','r');
t=text(tlm,yl(2),num2str(rndig(tlm*1000),'%g ms'),'Color','red');
