function [ELE,NODE,FACE]=loadmesh3d(meshname)

% LOADMESH - Load DCFEMLIB 3d ascii mesh
...
if ca,
    ...
    fclose(fid);
    %if spalten~=8, ELE=ELE(:,[1 1 2 3]);ELE(:,1)=(1:size(ELE,1))'; end
    fid=fopen([meshname '.n'],'r');
    zeile=fgetl(fid);
    fclose(fid);
    spalten=length(str2num(zeile));
    fid=fopen([meshname '.n'],'r');
    NODE=fscanf(fid,'%f',[spalten Inf])';
    %NODE(:,3)=abs(NODE(:,3));
    fclose(fid);
else
