function exportpng(fig, filename, varargin)
% EXPORTPNG - exports figure to png file
...
% varargin are additional (Property,Value) pairs, see writepng

if nargin==1, filename=fig;fig=gcf; end
col=get(fig,'Color');
set(fig,'Color',[1 1 1]);drawnow;
fra=getframe(fig);
data=fra.cdata;
data(1,1,:)=255;
while(length(unique(data(:,1,1)))==1), data(:,1,:)=[]; end
if isempty(data), return; end
while(length(unique(data(:,end,1)))==1), data(:,end,:)=[]; end
if isempty(data), return; end
while(length(unique(data(1,:,1)))==1), data(1,:,:)=[]; end
if isempty(data), return; end
while(length(unique(data(end,:,1)))==1), data(end,:,:)=[]; end
...
if (length(filename)<4)||(~isequal(lower(filename(end-3:end)),'.png')),
