...
C--------------------------------------------------------------
C     sens3dplane(x,y,z1,z2,pa,pm)
C
      integer plhs(*), prhs(*)
      integer mxCreateFull, mxGetPr
      integer xe_p, ye_p, pa_p, pm_p, out_p
      integer*4 nlhs, nrhs
      integer mxGetM, mxGetN, mxIsNumeric
      integer m, n, lx, ly, size
      integer i, j, k, nx, ny, ns, ne
      real*8 xe(100), ye(100), pa(3), pm(3)
      real*8 out(10000), iout, jout, kout
...
C     Check for proper number and size of arguments. 
      if(nrhs .ne. 6) then
         call mexErrMsgTxt('Six inputs arguments required.')
      ...
      plhs(1) = mxCreateFull(lx*ly, 1, 0)
      xe_p = mxGetPr(prhs(1))
      call mxCopyPtrToReal8(xe_p, xe, lx+1)
      ye_p = mxGetPr(prhs(2))
      call mxCopyPtrToReal8(ye_p, ye, ly+1)
      call mxCopyPtrToReal8(mxGetPr(prhs(3)), z1, 1)
      call mxCopyPtrToReal8(mxGetPr(prhs(4)), z2, 1)
      pa_p = mxGetPr(prhs(5))
...
20          continue
            iout=iout+jout*wg(i)
30        continue
          out(nx+(ny-1)*lx)=iout*dx*dy*dz/39.47841760435743
40      continue
50    continue

      call mxCopyReal8ToPtr(out, out_p, lx*ly)     
      return
      end
