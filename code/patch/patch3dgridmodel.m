function [cmin,cmax,iscb]=patch3dgridmodel(M,X,Y,Z,MAL,Bg,Alph)

% PATCH3DGRIDMODEL - Draw 3d Model M with X,Y,Z
...
% if ~isequal(Alph
clog=1;
if isfield(MAL,'xy'), xy=MAL.xy; else xy=0; end
...
if ~isequal(size(M),size(Alph)), Alph=ones(size(M)); end
%Orientierung
mm=M;
...
nr=0;
if clog&&isempty(cont), cont=log10(cont); end
switch cmap
    case 2, colmap=colormap(b2r);
    case 22, colmap=colormap(b2r);
        for i=1:size(colmap,1), colmap(i,:)=sqrt(mean(colmap(i,:))); end
...
caxis([cmin,cmax]);
ecolmap=colmap;
if (length(xx)<20)||(length(unique(mm(:)))<5), ecolmap(:)=0; end
...
while (nr<nu*nv)&(nr+startwith-1<lk),
    ...
    nn=nr+startwith-1;
    pp=mm(:,:,nn);
    cla reset;caxis([cmin cmax]);
    for i=1:size(pp,1),
        for k=1:size(pp,2),
            cind=1+round((pp(i,k)-cmin)/(cmax-cmin)*lcm);
            if cind>lcm, cind=lcm; end
            if cind<1, cind=1; end 
            patch(yy([k k+1 k+1 k]),xx([i i i+1 i+1]),...
                colmap(cind,:),'EdgeColor',ecolmap(cind,:),'FaceAlpha',Alph(i,k,nn),'EdgeAlpha',Alph(i,k,nn)); 
        end
    end
    if ~isempty(cont),
        hold on
    ...
    if vie==0, tit=sprintf('z=%g-%gm',Z(nn),Z(1+nn)); end
...
%         tit=sprintf('%s: %d\\Omegam',tit,round(Bg(nn)));  end
    ...
    % Testversion nächste 5 Z. einklammern
    global libmmfile
...
if nr<nu*nv, % one subplot left
    ...
    if strcmp(get(gcf,'Name'),'Sensitivity'), titel='sensitivity';
    else titel='\rho in \Omega m'; end
    if isfield(MAL,'canot'), titel=MAL.canot; end
    title(titel);
end
